...
export const ARQUETIPOS = [
  ...
  {
    id: "viuva_do_trono", nome: "A Viúva do Trono", peso: 2,
    nasceDe: (c, st) => (st.tratados || 0) > 0 && (st.dominios || 0) > 0,
    crenca: "o luto é o único juramento que não se quebra, e o trono me deve um",
    metodo: "governa por procuração: casa alianças, envenena sucessões, sorri em todo velório",
    quer: "sentar no lugar que tiraram da linhagem dela, e que ninguém mais chame aquilo de usurpação",
    assinaturas: ["o anel de um morto reaparecendo em cartas seladas", "viúvas recém-promovidas a cargos que não pediram", "brindes a um nome que ninguém ousa dizer alto"],
  },
  {
    id: "profeta_queimado", nome: "O Profeta Queimado", peso: 2,
    nasceDe: (c) => (c.decretosPregados || 0) > 0 || (c.decretosCumpridos || 0) > 0,
    crenca: "os deuses falaram comigo uma vez — e o mundo me queimou por ter ouvido",
    metodo: "converte os desesperados, anuncia datas, e faz a profecia se cumprir com as próprias mãos",
    quer: "que a cidade se ajoelhe no dia que ele marcou, nem que ele precise atear o fogo que profetizou",
    assinaturas: ["a mesma data riscada em três paredes distantes", "convertidos com a mesma queimadura no mesmo lugar", "um sermão que promete o fim e dá o endereço dele"],
  },
  {
    id: "crianca_antiga", nome: "A Criança Antiga", peso: 2,
    nasceDe: (c) => (c.masmorrasConcluidas || 0) >= 3,
    crenca: "eu já fui deus desta terra, e a terra não esqueceu — só vocês esqueceram",
    metodo: "fala pela boca de outros, cresce devagar, troca favores por fé de quem não entende o preço",
    quer: "acordar inteira no corpo certo, e o corpo certo ainda está sendo preparado",
    assinaturas: ["crianças cantando uma cantiga que ninguém ensinou", "poços que de repente adociram", "um santuário velho onde os bichos não entram"],
  },
  {
    id: "general_sem_guerra", nome: "O General Sem Guerra", peso: 2,
    nasceDe: (c, st) => (st.guerras || 0) > 0 || (c.combatesVencidos || 0) >= 15,
    crenca: "a paz apodrece os fortes — eu vi o que ela fez com os meus, e não vejo de novo",
    metodo: "fabrica incidentes de fronteira e vende a guerra como defesa antes que alguém note quem atirou primeiro",
    quer: "uma guerra de verdade, para os fortes voltarem a mandar e os fracos a agradecer",
    assinaturas: ["uniformes dos dois lados no mesmo baú", "veteranos recrutados de madrugada", "um ataque 'inimigo' com horário de expediente"],
  },
  {
    id: "boticaria", nome: "A Boticária", peso: 2,
    nasceDe: (c) => (c.quaseMorte || 0) >= 1 || (c.descansos || 0) >= 20,
    crenca: "quem cura decide quem vive — sempre foi assim, eu só fui a primeira a cobrar direito",
    metodo: "espalha o mal e vende o remédio; cada dose amarra mais um cliente à próxima",
    quer: "uma região inteira dependente da mão dela, e nenhum boticário rival de pé",
    assinaturas: ["a doença que só melhora com um fornecedor", "ervas que não são daquela terra na despensa dela", "os curados que ninguém reexamina"],
  },
  {
    id: "santo_vivo", nome: "O Santo Vivo", peso: 2,
    nasceDe: (c, st) => (st.fama || 0) >= 55,
    crenca: "milagre não admite concorrência, e o teu nome anda grande demais para o meu gosto",
    metodo: "caridade em praça, milagres encenados e difamação sussurrada de todo rival de fé",
    quer: "ser a única boca por onde o divino fala nesta região — e você virou boca demais",
    assinaturas: ["curados que somem antes de qualquer reexame", "a esmola que vem com uma condição", "um milagre sempre na hora em que a praça olhava"],
  },
  {
    id: "domador", nome: "O Domador", peso: 2,
    nasceDe: (c) => (c.elitesDerrotados || 0) >= 3 || (c.lendariosDerrotados || 0) >= 1,
    crenca: "o monstro é só uma ferramenta que os fracos temem — e medo, eu vendo bem",
    metodo: "solta a fera onde dói, e aparece logo depois oferecendo a proteção que só ele dá",
    quer: "que cada aldeia da região pague a ele para não soltar o que ele tem na coleira",
    assinaturas: ["ataques de bicho com hora marcada", "marcas de coleira em carcaças", "uma féra que recua ao assobio de um homem só"],
  },
  {
    id: "almiranta", nome: "A Almiranta", peso: 2,
    nasceDe: (c, st) => (st.tratados || 0) >= 2 || (c.viagens || 0) >= 8,
    crenca: "quem controla a travessia controla a fome — e a fome governa melhor que qualquer rei",
    metodo: "estrangula rotas, afunda quem não paga bandeira e chama de tempestade o que foi ordem dela",
    quer: "que nenhuma carga cruze a região sem passar pela mão dela primeiro",
    assinaturas: ["naufrágios sem tempestade", "portos onde o preço já vem combinado", "uma bandeira que todos os capitães reconhecem e ninguém descreve"],
  },
  {
