...
export const AGUARDAM = [
  ...
  { nome: "Esquiva Ágil", classe: "Ladino", promete: "anula o dano de um ataque por turno", motivo: "MUDOU DE DÍVIDA em F3 v9.280, e a que fica é a mesma de \"Corpo de Ferro\": a distância entre a ficção e o que a tabela paga. A habilidade atravessa a produção inteira — entrou em `GUARDAS` (habilidades.js) como `esquiva` de 1 turno, `erguerGuarda` a ergue nas duas mesas, `esquivaDeGuarda` entorta o dado em `resolverAtaque` e o prazo vence por `expirarGuardas`. MEDIDO em 600 golpes contra o mesmo alvo: 70,8% de acerto sem ela, 47,5% com ela — −32,9% de acerto e −37,9% de dano enquanto está de pé, que é a paridade de 2 PM desta casa (`absorve` a 2 PM come 4). O que NÃO sai é o \"anula\": zerar o golpe é `tipo: \"intocavel\"`, e a v9.53 decidiu que o absoluto custa 8 PM e dura um turno. Entregá-lo a 2 PM desfaria a escada pelo degrau mais barato — é decisão de equilíbrio escrita no cabeçalho de `GUARDAS`, não buraco — F3", desde: "16/09" },
  { nome: "Defesa Fluida", classe: "Monge", promete: "desvia do próximo ataque automaticamente", motivo: "MUDOU DE DÍVIDA em F3 v9.280, e é a irmã exata da de cima: entrou em `GUARDAS` como `esquiva` de 1 turno (2 PM, pela régua `ESCADA_DA_GUARDA`) e passa a entortar o dado de quem a ataca. Fica o \"automaticamente\", que é a mesma promessa absoluta da Esquiva Ágil e tem a mesma resposta da v9.53 — F3", desde: "16/09" },
  ...
  { nome: "Mente Serena", classe: "Monge", promete: "imune a medo e confusão por 3 turnos", motivo: "A DÍVIDA FOI MEDIDA em F3 v9.280 e é MAIOR do que esta linha dizia. Continua verdade que imunidade TEMPORÁRIA não existe (`imuneDeTraco` e `imuneA` são para sempre) — e a metade que faltava é esta: `imuneA` só é consultada em DOIS sítios, e os dois são autoinfligidos (`App.jsx:8085`, o preço do esforço, e `:15393`, a falha crítica). A aflição que um INIMIGO impõe ao herói entra por `rolarAflicao` (aflicoes.js), que não pergunta a `imuneA` coisa nenhuma — ou seja, nem a imunidade PERMANENTE do elmo Sem Medo ou da Coroa Lúcida protege hoje de um medo lançado contra você. Por isso esta não é etapa de prazo: é a porta única da aflição, e ela tem de vir antes. F3 pagou a outra metade da família `intocado` (o golpe que erra) e deixou esta escrita com o endereço — F3", desde: "16/09" },
