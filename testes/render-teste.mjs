...
// src/geografia.js
var PORTES = {
  ruina: { rotulo: "ru\xEDna", min: 0, max: 0, servicos: "nenhum \u2014 s\xF3 ecos e perigo" },
  ...
  cidade: { rotulo: "cidade", min: 1500, max: 15e3, servicos: "guildas, templos, mercado permanente" },
  capital: { rotulo: "capital", min: 15e3, max: 8e4, servicos: "corte, catedral, universidade, porto grande" },
  metropole: { rotulo: "metr\xF3pole", min: 8e4, max: 3e5, servicos: "tudo que o mundo oferece \u2014 e tudo que ele esconde" },
  fortaleza: { rotulo: "fortaleza", min: 200, max: 2e3, servicos: "guarni\xE7\xE3o, armeiro, poucas comodidades" }
};
function populacaoDe(porte, rnd = Math.random) {
  const p = PORTES[porte] || PORTES.cidade;
  if (!p.max) return 0;
  return Math.round((p.min + rnd() * (p.max - p.min)) / 10) * 10;
}
var BIOMAS = ["planicie", "floresta", "colina", "montanha", "deserto", "pantano", "costa", "gelo"];
var TERRENO_VIAGEM = {
  ...
  costa: { kmDia: 30, rotulo: "caminho litor\xE2neo" },
  pantano: { kmDia: 12, rotulo: "p\xE2ntano trai\xE7oeiro" },
  deserto: { kmDia: 15, rotulo: "deserto" },
  montanha: { kmDia: 12, rotulo: "passo de montanha" },
  gelo: { kmDia: 12, rotulo: "gelo" },
  maritima: { kmDia: 90, rotulo: "rota mar\xEDtima" }
};
var KM_POR_UNIDADE = 25;
var REGIAO_A = ["Terras", "Vales", "Campos", "Margens", "Colinas", "Ch\xE3s", "Fronteiras", "Planaltos"];
var REGIAO_B = ["do Corvo", "de Ferro", "das Brumas", "do Sal", "Verdes", "Altos", "Quebradas", "da Serpente", "do Vento", "Escarlates", "da Lua Baixa", "do Estio"];
var CIDADE_A = ["Pedra", "Vila", "Porto", "Forte", "Monte", "Rio", "Ponte", "Torre", "Alto", "Baixo", "Nova", "Velha", "Casa", "Ponto"];
var CIDADE_B = ["valente", "do Sul", "do Norte", "clara", "escura", "do Rei", "das \xC1guias", "Profundo", "da Fonte", "do Vigia", "Serena", "Rasa", "do Martelo", "das Velas", "Seco", "Brumoso"];
var CONT_A = ["Aeth", "Kor", "Vald", "Oss", "Thar", "Bel", "Myr", "Dur"];
var CONT_B = ["enia", "oria", "amar", "\xEAnia", "gard", "lon", "wic", "dor"];
...
var pickR = (rnd, arr) => arr[Math.floor(rnd() * arr.length)];
...
function gerarGeografia(semente) {
  const rnd = rngDe(semente);
  const continente = `${pickR(rnd, CONT_A)}${pickR(rnd, CONT_B)}`;
  const nRegioes = 3 + Math.floor(rnd() * 3);
  const usadosR = /* @__PURE__ */ new Set(), usadosC = /* @__PURE__ */ new Set();
  const regioes = [];
  for (let i = 0; i < nRegioes; i++) {
    let nome;
    do {
      nome = `${pickR(rnd, REGIAO_A)} ${pickR(rnd, REGIAO_B)}`;
    } while (usadosR.has(nome));
    usadosR.add(nome);
...
function centrosDeRegiao(cidades) {
  const grupos = {};
  (cidades || []).forEach((c) => {
    if (!c.regiao) return;
    (grupos[c.regiao] = grupos[c.regiao] || []).push(c);
  });
...
function blobPath(pontos, raio, semente = "blob") {
  const ps = (pontos || []).filter((p) => p && isFinite(p.x) && isFinite(p.y));
  if (!ps.length) return "";
  const rnd = rngDe2(semente);
...
// src/divindades.js
var NIVEL_DESPERTAR = 15;
var GRAUS = [
  { gd: 0, titulo: "Mortal", fieis: 0, desc: "Carne, tempo e medo \u2014 como quase todos." },
  { gd: 1, titulo: "Her\xF3i Lend\xE1rio", fieis: 1e3, desc: "O povo j\xE1 reza BAIXINHO o seu nome." },
  { gd: 2, titulo: "Semideus", fieis: 1e4, desc: "Uma centelha vive em voc\xEA \u2014 sua presen\xE7a pesa no ar." },
  { gd: 3, titulo: "Divindade Menor", fieis: 1e5, desc: "Um dom\xEDnio responde quando voc\xEA chama." },
  { gd: 4, titulo: "Divindade Maior", fieis: 1e6, desc: "For\xE7a c\xF3smica \u2014 o mundo se curva." }
];
function grauDe(div) {
...
function tituloDe(gd) {
  return (GRAUS[Math.max(0, Math.min(4, gd || 0))] || GRAUS[0]).titulo;
}
function proximoPatamar(div) {
  const gd = grauDe(div);
  if (gd >= 4) return null;
...
function bonusDivino(gdAtacante, gdAlvo) {
  const dif = (gdAtacante || 0) - (gdAlvo || 0);
  return dif * 2;
}
function imunePorEscopo(gdAtacante, gdAlvo) {
  return (gdAlvo || 0) - (gdAtacante || 0) >= 3;
}
function garantirDivindade(dv2) {
  const d = dv2 && typeof dv2 === "object" ? dv2 : {};
  return {
    ehJogador: true,
    despertar: !!d.despertar,
    fieis: Math.max(0, Number(d.fieis) || 0),
    pf: Math.max(0, Number(d.pf) || 0),
    // Pontos de Fé (combustível de milagres)
    dominio: d.dominio || "",
    // o conceito que o herói encarnará
    patrono: d.patrono || "",
    // deus patrono, se houver
    estagio: d.estagio || "",
    // servo | semideus | divindade (rotulo narrativo)
    panteao: Array.isArray(d.panteao) ? d.panteao : [],
    // divindades conhecidas do mundo
    seq: Number(d.seq) || 1
  };
}
var DIV_NOMES_A = ["Vhar", "Oss", "Mel", "Thar", "Ilu", "Kael", "Zor", "Ael", "Mor", "Sae", "Dun", "Rha", "Vel", "Nyss", "Oth", "Bren"];
var DIV_NOMES_B = ["ath", "imir", "andra", "ok", "is", "ael", "un", "eira", "oth", "yra", "ash", "iel", "om", "essa"];
var DOMINIOS = [
  { dominio: "da Colheita e da Fome", icone: "\u{1F33E}", temper: ["generosa", "cobradora", "silenciosa"] },
  { dominio: "das Tempestades", icone: "\u26C8", temper: ["col\xE9rica", "imprevis\xEDvel", "orgulhosa"] },
  { dominio: "dos Mortos e dos Juramentos", icone: "\u{1F480}", temper: ["solene", "vingativa", "justa"] },
  { dominio: "da Forja e do Fogo", icone: "\u{1F525}", temper: ["teimosa", "honrada", "ciumenta"] },
  { dominio: "dos Mares e das Perdi\xE7\xF5es", icone: "\u{1F30A}", temper: ["vol\xFAvel", "ciumenta", "antiga"] },
  { dominio: "da Lua e dos Segredos", icone: "\u{1F319}", temper: ["enigm\xE1tica", "gentil", "mentirosa"] },
  { dominio: "da Guerra e das Correntes", icone: "\u26D3", temper: ["brutal", "honrada", "faminta"] },
  { dominio: "das Bestas e das Fronteiras", icone: "\u{1F43A}", temper: ["selvagem", "protetora", "desconfiada"] },
  { dominio: "do Com\xE9rcio e dos Cifr\xF5es", icone: "\u2696", temper: ["negociadora", "avara", "elegante"] },
  { dominio: "dos Sonhos e dos Pesadelos", icone: "\u{1F56F}", temper: ["t\xEAnue", "perturbadora", "maternal"] },
  { dominio: "da Doen\xE7a e da Cura", icone: "\u{1FA78}", temper: ["amb\xEDgua", "fria", "compassiva"] },
  { dominio: "do Conhecimento Proibido", icone: "\u{1F4D6}", temper: ["curiosa", "perigosa", "paciente"] }
];
var TITULOS_CULTO = ["a Ordem", "o C\xEDrculo", "os Filhos", "a Seita", "os Vigias", "o Conc\xEDlio", "as M\xE3os"];
function nomeDivino() {
  return DIV_NOMES_A[Math.floor(Math.random() * DIV_NOMES_A.length)] + DIV_NOMES_B[Math.floor(Math.random() * DIV_NOMES_B.length)];
}
function gerarDivindade(ctx, dia, { gdFixo = null } = {}) {
  const dom = DOMINIOS[Math.floor(Math.random() * DOMINIOS.length)];
  const rolo = Math.random();
  const gd = gdFixo != null ? gdFixo : rolo < 0.4 ? 2 : rolo < 0.75 ? 3 : 4;
  const baseFieis = GRAUS[gd].fieis || 5e3;
  const fieis = gd === 2 ? 5e3 + Math.floor(Math.random() * 45e3) : gd === 3 ? 1e5 + Math.floor(Math.random() * 4e5) : 1e6 + Math.floor(Math.random() * 3e6);
  ...
  return {
    ...
    gd,
    fieis,
    pf: Math.floor(fieis / 1e3),
    // reserva de milagres proporcional à fé
    temperamento: temper,
    ...
    criadoEm: dia,
    nota: `${temper[0].toUpperCase()}${temper.slice(1)} \u2014 culto ${gd >= 4 ? "antigo e presente em todo o mundo conhecido" : gd === 3 ? "forte em v\xE1rias cidades" : "local, mas fervoroso"}.`
  };
}
function gerarPanteaoInicial(ctx, dia) {
  return [
...
function pfPorDia(div) {
  const f = div && div.fieis || 0;
  if (!f) return 0;
  return Math.max(1, Math.round(Math.sqrt(f) / 3));
}
function pfMaximo(div) {
  const f = div && div.fieis || 0;
...
var MILAGRES = [
  {
    id: "bencao",
    nome: "B\xEAn\xE7\xE3o",
    icone: "\u2728",
    pf: 5,
    gd: 1,
    alvo: "grupo",
    desc: "Vantagem nas pr\xF3ximas rolagens de todo o grupo.",
    efeito: { tipo: "efeito", nome: "Aben\xE7oado", bonus: 2, turnos: 5, aplica: "todos" }
  },
  {
    id: "cura",
    ...
    gd: 1,
    alvo: "grupo",
    desc: "Restaura a vida do her\xF3i e dos companheiros.",
    efeito: { tipo: "cura", fracao: 0.5 }
  },
  {
    id: "presagio",
    nome: "Press\xE1gio",
    ...
    gd: 1,
    alvo: "mundo",
    desc: "O dom\xEDnio revela o que est\xE1 por vir \u2014 uma verdade oculta da cena.",
    efeito: { tipo: "revelacao" }
  },
  {
    id: "juramento",
    nome: "Juramento Selado",
    icone: "\u26D3",
    pf: 15,
    gd: 2,
    alvo: "npc",
    desc: "Um NPC presente jura lealdade \u2014 v\xEDnculo sobe ao m\xE1ximo.",
    efeito: { tipo: "vinculo", valor: 100 }
  },
  {
    id: "furia",
    nome: "F\xFAria do Dom\xEDnio",
    icone: "\u26A1",
    pf: 20,
    gd: 2,
    alvo: "combate",
    desc: "O poder desaba sobre os inimigos: dano pesado em todos.",
    efeito: { tipo: "dano_area", fracao: 0.35 }
  },
  {
    id: "refugio",
    nome: "Ref\xFAgio Sagrado",
    icone: "\u{1F6E1}",
    pf: 18,
    gd: 2,
    alvo: "grupo",
    desc: "Ningu\xE9m do grupo cai neste combate \u2014 o golpe fatal \xE9 negado uma vez.",
    efeito: { tipo: "efeito", nome: "Ref\xFAgio", bonus: 0, turnos: 6, aplica: "todos" }
  },
  {
    id: "ressurgir",
    nome: "Chamar de Volta",
    icone: "\u{1F56F}",
    pf: 50,
    gd: 3,
    alvo: "grupo",
    desc: "Traz de volta quem tombou \u2014 a morte devolve o que \xE9 seu.",
    efeito: { tipo: "ressurreicao" }
  },
  {
    id: "decreto",
    nome: "Decreto Divino",
    ...
    gd: 3,
    alvo: "mundo",
    desc: "Sua vontade vira lei natural numa regi\xE3o: o mundo obedece.",
    efeito: { tipo: "mundo" }
  },
  {
    id: "avatar",
    nome: "Manifesta\xE7\xE3o",
    icone: "\u{1F31F}",
    pf: 80,
    gd: 4,
    alvo: "mundo",
    desc: "Voc\xEA se manifesta em pessoa a todos de uma cidade inteira.",
    efeito: { tipo: "mundo", fe: "marco" }
  }
];

// src/devocao.js
function rngDe3(semente) {
  let h = 2166136261;
...
var TEMPLOS = [
  null,
  { nivel: 1, nome: "Santu\xE1rio", icone: "\u{1F56F}", custo: 150, feDia: 0.6, pf: 1, gd: 0, felicidade: 2, desc: "Uma pedra, um nome gravado e velas que ningu\xE9m apaga." },
  { nivel: 2, nome: "Templo", icone: "\u26EA", custo: 500, feDia: 1.2, pf: 2, gd: 1, felicidade: 4, desc: "Portas abertas, sacerd\xF3cio pr\xF3prio e of\xEDcios di\xE1rios." },
  { nivel: 3, nome: "Catedral", icone: "\u{1F3DB}", custo: 1500, feDia: 2, pf: 4, gd: 2, felicidade: 7, desc: "A cidade inteira ouve os sinos \u2014 e sabe de quem s\xE3o." }
];
var TEMPLO_MAX = 3;
function temploDe(nivel) {
  return TEMPLOS[Math.max(0, Math.min(TEMPLO_MAX, nivel || 0))] || null;
}
function entradaVazia() {
  return { fe: 0, templo: 0, desde: 0 };
}
var arred = (x) => Math.round(x * 1e4) / 1e4;
function feDaCidade(dev2, nome) {
  const e = dev2 && dev2.cidades && dev2.cidades[nome] || null;
  return e ? Math.max(0, Math.min(100, e.fe || 0)) : 0;
}
function temploDaCidade(dev2, nome) {
  const e = dev2 && dev2.cidades && dev2.cidades[nome] || null;
  return e ? Math.max(0, Math.min(TEMPLO_MAX, e.templo || 0)) : 0;
}
function patronoDaCidade(cidade, panteao2) {
  const lista = (panteao2 || []).filter((d) => d && d.nome);
  if (!lista.length || !cidade) return null;
  const rnd = rngDe3(`patrono|${cidade.nome}`);
  return lista[Math.floor(rnd() * lista.length)] || lista[0];
}
function fervorDaCidade(cidade) {
  if (!cidade) return 0;
  const rnd = rngDe3(`fervor|${cidade.nome}`);
  const porte = cidade.porte || cidade.tipo || "cidade";
  const ajuste = porte === "capital" || porte === "metropole" ? 12 : porte === "cidade" ? 4 : porte === "fortaleza" ? -12 : porte === "ruina" ? -30 : 0;
  return Math.max(0, Math.min(90, Math.round(28 + rnd() * 40 + ajuste)));
}
function heresiaDaCidade(cidade, dev2) {
  return Math.max(0, fervorDaCidade(cidade) - feDaCidade(dev2, cidade.nome));
}
var ESTADOS_FE = {
  santa: { chave: "santa", rotulo: "Cidade Santa", cor: "#F5C878", icone: "\u{1F31F}", recepcao: "recebem o her\xF3i com hinos, prociss\xE3o e joelhos no ch\xE3o" },
  devota: { chave: "devota", rotulo: "Devota", cor: "#E8A33D", icone: "\u{1F64F}", recepcao: "h\xE1 preces em seu nome nas casas e uma multid\xE3o que o reconhece na rua" },
  simpatica: { chave: "simpatica", rotulo: "Simp\xE1tica", cor: "#7BC98F", icone: "\u{1F56F}", recepcao: "alguns j\xE1 rezam para o her\xF3i, a maioria s\xF3 ouviu falar" },
  indiferente: { chave: "indiferente", rotulo: "Indiferente", cor: "#9A93A6", icone: "\xB7", recepcao: "o nome do her\xF3i n\xE3o significa nada aqui" },
  herege: { chave: "herege", rotulo: "Herege", cor: "#D86A5B", icone: "\u26A0", recepcao: "o culto local trata o her\xF3i como falso profeta \u2014 serm\xF5es contra, portas fechadas" },
  hostil: { chave: "hostil", rotulo: "Terra Hostil", cor: "#C0504D", icone: "\u{1F525}", recepcao: "pregar o nome do her\xF3i aqui \xE9 blasf\xEAmia pun\xEDvel \u2014 pedras, den\xFAncia e fogueira" }
};
function estadoFe(cidade, dev2) {
  const fe = feDaCidade(dev2, cidade.nome);
  const her = heresiaDaCidade(cidade, dev2);
...
function fieisDaCidade(cidade, dev2) {
  return Math.round((cidade.populacao || 0) * feDaCidade(dev2, cidade.nome) / 100);
}
function fieisTotais(mapa2, dev2) {
  const cidades = mapa2 && mapa2.cidades || [];
  const emCidades = cidades.reduce((s, c) => s + fieisDaCidade(c, dev2), 0);
  return emCidades + Math.max(0, Math.round(dev2 && dev2.andarilhos || 0));
}
function resumoNumerico(mapa2, dev2) {
  const cidades = mapa2 && mapa2.cidades || [];
  const comFe = cidades.filter((c) => feDaCidade(dev2, c.nome) > 0);
  return {
    fieis: fieisTotais(mapa2, dev2),
    andarilhos: Math.max(0, Math.round(dev2 && dev2.andarilhos || 0)),
    lugares: comFe.length,
    santas: cidades.filter((c) => estadoFe(c, dev2).chave === "santa").length,
    heregias: cidades.filter((c) => ["herege", "hostil"].includes(estadoFe(c, dev2).chave)).length,
    templos: cidades.reduce((s, c) => s + (temploDaCidade(dev2, c.nome) > 0 ? 1 : 0), 0),
    feDia: cidades.reduce((s, c) => s + ((temploDe(temploDaCidade(dev2, c.nome)) || {}).pf || 0), 0)
  };
}
function garantirDevocao(dev2, mapa2, divindade) {
  const cidades = mapa2 && mapa2.cidades || [];
  const base = dev2 && typeof dev2 === "object" && dev2.cidades && typeof dev2.cidades === "object" ? { cidades: { ...dev2.cidades }, andarilhos: Math.max(0, Math.round(dev2.andarilhos || 0)) } : null;
  if (base) {
    for (const c of cidades) {
      const e = base.cidades[c.nome];
      base.cidades[c.nome] = e && typeof e === "object" ? { fe: Math.max(0, Math.min(100, Number(e.fe) || 0)), templo: Math.max(0, Math.min(TEMPLO_MAX, Number(e.templo) || 0)), desde: Number(e.desde) || 0 } : entradaVazia();
    }
    return base;
  }
  ...
  return espalharFieis(novo, mapa2, fieisAntigos);
}
function espalharFieis(dev2, mapa2, fieis, tetoPct = 55) {
  const total = Math.max(0, Math.round(fieis || 0));
  if (!total) return dev2;
  const cidades = mapa2 && mapa2.cidades || [];
  const popTotal = cidades.reduce((s, c) => s + (c.populacao || 0), 0);
  if (!cidades.length || popTotal <= 0) return { ...dev2, andarilhos: (dev2.andarilhos || 0) + total };
  const d = { cidades: { ...dev2.cidades || {} }, andarilhos: dev2.andarilhos || 0 };
  let colocados = 0;
  for (const c of cidades) {
    ...
    const teto = Math.max(tetoPct, feAtual);
...
function ganharDevocao(dev2, nomeCidade2, pontos, dia) {
  const d = { cidades: { ...dev2.cidades || {} }, andarilhos: dev2.andarilhos || 0 };
  const atual = d.cidades[nomeCidade2] || entradaVazia();
  const fe = Math.max(0, Math.min(100, (atual.fe || 0) + pontos));
  d.cidades[nomeCidade2] = { ...atual, fe: arred(fe), desde: atual.desde || dia || 0 };
  return d;
}
function fieisParaPontos(cidade, fieis) {
  const pop = cidade && cidade.populacao || 0;
  if (!pop) return 0;
  return Math.max(0, fieis / pop * 100);
}
function depositarFieis(dev2, mapa2, nomeLocal, fieis, dia) {
  const total = Math.max(0, Math.round(fieis || 0));
  const alvo = acharCidade(mapa2 && mapa2.cidades || [], nomeLocal);
  if (!alvo || !alvo.populacao) {
    return { devocao: { ...dev2, andarilhos: Math.max(0, (dev2.andarilhos || 0) + total) }, cidade: null, pontos: 0, sobra: total, estadoAntes: null, estadoDepois: null };
  }
  const antes = feDaCidade(dev2, alvo.nome);
  const estadoAntes = estadoFe(alvo, dev2);
  ...
  let d = ganharDevocao(dev2, alvo.nome, depois - antes, dia);
...
function acharCidade(cidades, nome) {
  if (!nome) return null;
  const alvo = String(nome).toLowerCase().trim();
  const cs = cidades || [];
  return cs.find((c) => (c.nome || "").toLowerCase() === alvo) || cs.find((c) => alvo.includes((c.nome || "").toLowerCase()) || (c.nome || "").toLowerCase().includes(alvo)) || null;
}
function erguerTemplo(dev2, nomeCidade2, dia) {
  const d = { cidades: { ...dev2.cidades || {} }, andarilhos: dev2.andarilhos || 0 };
  const atual = d.cidades[nomeCidade2] || entradaVazia();
  const nivel = Math.min(TEMPLO_MAX, (atual.templo || 0) + 1);
  const t = temploDe(nivel);
  const salto = nivel === 1 ? 6 : nivel === 2 ? 10 : 15;
  d.cidades[nomeCidade2] = {
    ...atual,
    templo: nivel,
...
// src/painel-mapa.jsx
function PainelMapa({ mapa: mapa2, faccaoJogador, cidadeAtual, devocao, divindade }) {
  const [selecionada, setSelecionada] = import_react.default.useState(null);
  const desperto = !!(divindade && divindade.despertar);
  const [camada, setCamada] = import_react.default.useState("politica");
  const verFe = desperto && camada === "fe";
  const dev2 = devocao || { cidades: {} };
  const panteao2 = divindade && divindade.panteao || [];
  const numFe = desperto ? resumoNumerico(mapa2, dev2) : null;
  const corDaCidade = (c) => verFe ? estadoFe(c, dev2).cor : (RELACOES[c.relacao] || RELACOES.neutra).cor;
  const cidades = mapa2?.cidades || [];
  const faccoes = mapa2?.faccoes || [];
  const dominadas = cidades.filter((c) => c.relacao === "jogador").length;
  const regioes = [...new Set(cidades.map((c) => c.regiao).filter(Boolean))];
  const gruposRegiao = {};
  cidades.forEach((c) => {
    if (c.regiao) (gruposRegiao[c.regiao] = gruposRegiao[c.regiao] || []).push(c);
  });
  if (cidades.length === 0) {
    return /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-sm italic", style: { color: T.inkDim } }, "O mapa ainda est\xE1 em branco. Conforme voc\xEA explora, cidades e territ\xF3rios aparecem aqui \u2014 e ficam salvos, para o mundo nunca mais se perder.");
  }
  return /* @__PURE__ */ import_react.default.createElement("div", null, faccaoJogador && /* @__PURE__ */ import_react.default.createElement("div", { className: "rounded-xl p-3 mb-3", style: { background: T.panelSoft, border: `1px solid ${T.amber}` } }, /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest", style: { color: T.amberSoft } }, "Sua fac\xE7\xE3o"), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-display text-xl", style: { color: T.ink } }, faccaoJogador), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-xs", style: { color: T.inkDim } }, "Domina ", dominadas, " ", dominadas === 1 ? "cidade" : "cidades", ".")), desperto && /* @__PURE__ */ import_react.default.createElement("div", { className: "flex gap-1.5 mb-2" }, [{ id: "politica", rotulo: "\u{1F3F3} Pol\xEDtica" }, { id: "fe", rotulo: "\u{1F64F} F\xE9" }].map((k) => /* @__PURE__ */ import_react.default.createElement(
    "button",
    {
      key: k.id,
      onClick: () => setCamada(k.id),
    ...
    k.rotulo
  ))), verFe && numFe && /* @__PURE__ */ import_react.default.createElement("div", { className: "rounded-xl p-3 mb-3", style: { background: T.panelSoft, border: `1px solid ${T.violet}` } }, /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest", style: { color: T.violetSoft } }, "Onde o seu nome \xE9 rezado"), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-display text-xl", style: { color: T.ink } }, numFe.fieis.toLocaleString("pt-BR"), " fi\xE9is"), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-xs", style: { color: T.inkDim } }, numFe.lugares, " lugar(es) \xB7 ", numFe.templos, " templo(s) \xB7 ", numFe.santas, " cidade(s) santa(s) \xB7 ", numFe.heregias, " onde voc\xEA \xE9 herege", numFe.andarilhos ? ` \xB7 ${numFe.andarilhos.toLocaleString("pt-BR")} andarilhos sem cidade` : "")), /* @__PURE__ */ import_react.default.createElement("div", { className: "relative rounded-xl mb-3", style: { border: `1px solid ${T.line}`, aspectRatio: "4 / 3", overflow: "hidden", background: "#96b7ae" } }, /* @__PURE__ */ import_react.default.createElement("svg", { viewBox: "0 0 100 100", preserveAspectRatio: "none", style: { position: "absolute", inset: 0, width: "100%", height: "100%" } }, /* @__PURE__ */ import_react.default.createElement("defs", null, /* @__PURE__ */ import_react.default.createElement("filter", { id: "tvPapel" }, /* @__PURE__ */ import_react.default.createElement("feTurbulence", { type: "fractalNoise", baseFrequency: "0.9", numOctaves: "2", result: "n" }), /* @__PURE__ */ import_react.default.createElement("feColorMatrix", { in: "n", type: "matrix", values: "0 0 0 0 0.45 0 0 0 0 0.38 0 0 0 0 0.26 0 0 0 0.28 0" })), /* @__PURE__ */ import_react.default.createElement("filter", { id: "tvCosta", x: "-25%", y: "-25%", width: "150%", height: "150%" }, /* @__PURE__ */ import_react.default.createElement("feTurbulence", { type: "fractalNoise", baseFrequency: "0.055", numOctaves: "3", seed: "7", result: "t" }), /* @__PURE__ */ import_react.default.createElement("feDisplacementMap", { in: "SourceGraphic", in2: "t", scale: "3.2" }))), /* @__PURE__ */ import_react.default.createElement("rect", { x: "0", y: "0", width: "100", height: "100", fill: "#96b7ae" }), cidades.length > 0 && (() => {
    const dCont = blobPath(cidades, 17, "continente|" + cidades.length);
    return /* @__PURE__ */ import_react.default.createElement("g", { filter: "url(#tvCosta)" }, /* @__PURE__ */ import_react.default.createElement("path", { d: dCont, fill: "none", stroke: "#f0e7cf", strokeWidth: "2.6", opacity: "0.55" }), /* @__PURE__ */ import_react.default.createElement("path", { d: dCont, fill: "#eadfc1", stroke: "#6d5c40", strokeWidth: "0.5" }));
  })(), Object.entries(gruposRegiao).map(([nomeR, csR], i) => {
    const cor = verFe ? corDaCidade([...csR].sort((a, b) => feDaCidade(dev2, b.nome) - feDaCidade(dev2, a.nome))[0]) : (centrosDeRegiao(csR)[0] || {}).cor || "#9A93A6";
    return /* @__PURE__ */ import_react.default.createElement("g", { key: `terr-${i}`, filter: "url(#tvCosta)" }, /* @__PURE__ */ import_react.default.createElement("path", { d: blobPath(csR, 8.5, "regiao|" + nomeR), fill: cor, opacity: "0.14", stroke: "#6d5c40", strokeOpacity: "0.55", strokeWidth: "0.35", strokeDasharray: "1.6 1.2" }));
  }), gerarEstradas(cidades).map((rt, i) => {
    const a = cidades[rt.a], b = cidades[rt.b];
    if (!a || !b) return null;
    return /* @__PURE__ */ import_react.default.createElement("line", { key: `rd-${i}`, x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: "#7a5f3d", strokeOpacity: rt.mesmaRegiao ? 0.55 : 0.35, strokeWidth: rt.mesmaRegiao ? 0.5 : 0.4, strokeDasharray: rt.mesmaRegiao ? "" : "1.2 1.2" });
  }), centrosDeRegiao(cidades).map((r, i) => /* @__PURE__ */ import_react.default.createElement("g", { key: `mt-${i}`, stroke: "#6d5c40", strokeWidth: "0.4", fill: "none", opacity: "0.6" }, /* @__PURE__ */ import_react.default.createElement("path", { d: `M ${r.x - 4.4} ${r.y - 4.6} L ${r.x - 3} ${r.y - 6.6} L ${r.x - 1.6} ${r.y - 4.6}` }), /* @__PURE__ */ import_react.default.createElement("path", { d: `M ${r.x - 1.2} ${r.y - 4.4} L ${r.x + 0.4} ${r.y - 6.9} L ${r.x + 2} ${r.y - 4.4}` }), /* @__PURE__ */ import_react.default.createElement("path", { d: `M ${r.x + 2.4} ${r.y - 4.6} L ${r.x + 3.8} ${r.y - 6.4} L ${r.x + 5.2} ${r.y - 4.6}` }))), /* @__PURE__ */ import_react.default.createElement("rect", { x: "0", y: "0", width: "100", height: "100", filter: "url(#tvPapel)", opacity: "0.55" }), /* @__PURE__ */ import_react.default.createElement("g", { transform: "translate(90.5,88)" }, /* @__PURE__ */ import_react.default.createElement("path", { d: "M0,-6.2 L1.3,-1.3 L6.2,0 L1.3,1.3 L0,6.2 L-1.3,1.3 L-6.2,0 L-1.3,-1.3 Z", fill: "#5c4a30", opacity: "0.85" }), /* @__PURE__ */ import_react.default.createElement("path", { d: "M0,-3.6 L0.9,-0.9 L3.6,0 L0.9,0.9 L0,3.6 L-0.9,0.9 L-3.6,0 L-0.9,-0.9 Z", fill: "#c9a45a", transform: "rotate(45)", opacity: "0.9" }), /* @__PURE__ */ import_react.default.createElement("circle", { r: "0.7", fill: "#eadfc1" })), /* @__PURE__ */ import_react.default.createElement("g", { transform: "translate(6,93.5)" }, [0, 1, 2, 3].map((k) => /* @__PURE__ */ import_react.default.createElement("rect", { key: k, x: k * 5, y: "0", width: "5", height: "1.1", fill: k % 2 ? "#eadfc1" : "#5c4a30", stroke: "#5c4a30", strokeWidth: "0.15" }))), /* @__PURE__ */ import_react.default.createElement("rect", { x: "0.8", y: "0.8", width: "98.4", height: "98.4", fill: "none", stroke: "#5c4a30", strokeWidth: "0.7", opacity: "0.8" }), /* @__PURE__ */ import_react.default.createElement("rect", { x: "2.2", y: "2.2", width: "95.6", height: "95.6", fill: "none", stroke: "#5c4a30", strokeWidth: "0.25", opacity: "0.6" })), centrosDeRegiao(cidades).map((r, i) => /* @__PURE__ */ import_react.default.createElement("div", { key: `rn-${i}`, style: { position: "absolute", left: `${r.x}%`, top: `${r.y - 10}%`, transform: "translate(-50%,-50%)", pointerEvents: "none" } }, /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-display", style: { fontSize: 12, color: "#5c4a30", opacity: 0.75, letterSpacing: "0.06em", whiteSpace: "nowrap", textShadow: "0 1px 1px #f0e6cc" } }, r.regiao))), cidades.map((c, i) => {
    const atual = cidadeAtual && c.nome.toLowerCase() === String(cidadeAtual).toLowerCase();
    const cor = corDaCidade(c);
    const est = verFe ? estadoFe(c, dev2) : null;
    const templo = verFe ? temploDe(temploDaCidade(dev2, c.nome)) : null;
    const halo = est && (est.chave === "santa" || est.chave === "devota") ? `0 0 10px ${est.cor}` : est && (est.chave === "hostil" || est.chave === "herege") ? `0 0 8px ${est.cor}` : atual || selecionada === c.nome ? "0 0 8px #c9a45a" : "0 1px 2px #00000040";
    return /* @__PURE__ */ import_react.default.createElement("div", { key: i, style: { position: "absolute", left: `${c.x}%`, top: `${c.y}%`, transform: "translate(-50%,-50%)", textAlign: "center" } }, /* @__PURE__ */ import_react.default.createElement("div", { onClick: () => setSelecionada(selecionada === c.nome ? null : c.nome), style: { width: c.sede ? 15 : 10, height: c.sede ? 15 : 10, borderRadius: c.tipo === "capital" || c.sede ? 3 : "50%", background: cor, border: atual || selecionada === c.nome ? `2px solid #3a2e1c` : `1.5px solid #3a2e1c`, boxShadow: halo, margin: "0 auto", cursor: "pointer" } }), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-mono", style: { fontSize: 7, color: "#3a2e1c", marginTop: 1, whiteSpace: "nowrap", fontWeight: 600, textShadow: "0 1px 2px #f0e6cc, 0 -1px 2px #f0e6cc" } }, templo ? `${templo.icone} ` : "", c.nome, c.sede ? " \u2605" : "", verFe && feDaCidade(dev2, c.nome) >= 8 ? ` ${Math.round(feDaCidade(dev2, c.nome))}%` : ""));
  })), /* @__PURE__ */ import_react.default.createElement("div", { className: "flex flex-wrap gap-2 mb-3" }, (verFe ? Object.entries(ESTADOS_FE) : Object.entries(RELACOES)).map(([k, v]) => /* @__PURE__ */ import_react.default.createElement("div", { key: k, className: "flex items-center gap-1" }, /* @__PURE__ */ import_react.default.createElement("span", { style: { width: 8, height: 8, borderRadius: "50%", background: v.cor, display: "inline-block" } }), /* @__PURE__ */ import_react.default.createElement("span", { className: "tv-mono text-[9px]", style: { color: T.inkDim } }, v.rotulo)))), faccoes.length > 0 && /* @__PURE__ */ import_react.default.createElement("div", { className: "mb-3" }, /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-1.5", style: { color: T.inkDim } }, "Fac\xE7\xF5es"), /* @__PURE__ */ import_react.default.createElement("div", { className: "space-y-1.5" }, faccoes.map((f, i) => {
    const rel = RELACOES[f.relacao] || RELACOES.neutra;
    return /* @__PURE__ */ import_react.default.createElement("div", { key: i, className: "rounded-lg px-3 py-2 flex items-center justify-between gap-2", style: { background: T.panelSoft, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react.default.createElement("div", null, /* @__PURE__ */ import_react.default.createElement("span", { className: "tv-body text-sm", style: { color: T.ink } }, f.nome), f.lider ? /* @__PURE__ */ import_react.default.createElement("span", { className: "tv-body text-xs", style: { color: T.inkDim } }, " \xB7 ", f.lider) : null), /* @__PURE__ */ import_react.default.createElement("span", { className: "tv-mono text-[9px] px-1.5 py-0.5 rounded shrink-0", style: { color: rel.cor, border: `1px solid ${rel.cor}` } }, rel.rotulo));
  }))), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-1.5", style: { color: T.inkDim } }, "Cidades (", cidades.length, ")"), /* @__PURE__ */ import_react.default.createElement("div", { className: "space-y-2" }, cidades.map((c, i) => {
    const rel = RELACOES[c.relacao] || RELACOES.neutra;
    const aberta = selecionada === c.nome;
    const atual = cidadeAtual && c.nome.toLowerCase() === String(cidadeAtual).toLowerCase();
    const est = desperto ? estadoFe(c, dev2) : null;
    const fe = desperto ? feDaCidade(dev2, c.nome) : 0;
    const templo = desperto ? temploDe(temploDaCidade(dev2, c.nome)) : null;
    const etiqueta = verFe && est ? { rotulo: est.rotulo, cor: est.cor } : { rotulo: rel.rotulo, cor: rel.cor };
    const patrono = desperto ? patronoDaCidade(c, panteao2) : null;
    return /* @__PURE__ */ import_react.default.createElement("div", { key: i, className: "rounded-lg px-3 py-2", style: { background: T.panelSoft, border: `1px solid ${aberta ? T.amber : atual ? T.amberSoft : T.line}`, cursor: "pointer" }, onClick: () => setSelecionada(aberta ? null : c.nome) }, /* @__PURE__ */ import_react.default.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ import_react.default.createElement("span", { className: "tv-body text-sm", style: { color: T.ink } }, c.sede ? "\u2605 " : "", atual ? "\u{1F4CD} " : "", templo ? `${templo.icone} ` : "", c.nome), /* @__PURE__ */ import_react.default.createElement("span", { className: "tv-mono text-[9px] px-1.5 py-0.5 rounded shrink-0", style: { color: etiqueta.cor, border: `1px solid ${etiqueta.cor}` } }, etiqueta.rotulo)), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-xs mt-0.5", style: { color: T.inkDim } }, c.tipo, c.regiao ? ` \xB7 ${c.regiao}` : "", c.faccao ? ` \xB7 ${c.faccao}` : ""), verFe && /* @__PURE__ */ import_react.default.createElement("div", { className: "mt-1.5" }, /* @__PURE__ */ import_react.default.createElement("div", { className: "h-1 rounded-full overflow-hidden", style: { background: T.panel } }, /* @__PURE__ */ import_react.default.createElement("div", { className: "h-full rounded-full", style: { width: `${Math.round(fe)}%`, background: est.cor } })), /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-mono text-[9px] mt-0.5", style: { color: T.inkDim } }, Math.round(fe), "% devotos \xB7 \u2248", fieisDaCidade(c, dev2).toLocaleString("pt-BR"), " fi\xE9is", templo ? ` \xB7 ${templo.nome}` : "", patrono && heresiaDaCidade(c, dev2) >= 25 ? ` \xB7 culto de ${patrono.nome} resiste (${heresiaDaCidade(c, dev2)}%)` : "")), aberta && (verFe || c.notas || (c.locais || []).length > 0) && /* @__PURE__ */ import_react.default.createElement("div", { className: "mt-2 pt-2", style: { borderTop: `1px solid ${T.line}` } }, verFe && est && /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-xs mb-1", style: { color: est.cor } }, est.icone, " Ao chegar: ", est.recepcao, "."), c.notas && /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-xs", style: { color: T.inkDim } }, c.notas), (c.locais || []).length > 0 && /* @__PURE__ */ import_react.default.createElement("div", { className: "tv-body text-xs mt-1", style: { color: T.violetSoft } }, "Locais: ", c.locais.join(", "))));
  })));
}

// src/painel-ascensao.jsx
var import_react2 = __toESM(require_react(), 1);
function PainelAscensao({ divindade, nivel, onDespertar, onRecalibrar, recalibrando, onMilagre, mapa: mapa2, devocao }) {
  const dv2 = divindade || garantirDivindade(null);
  const gd = grauDe(dv2);
  const prox = proximoPatamar(dv2);
  ...
  const comparar = (gdOutro) => {
    ...
    return { txt: "equil\xEDbrio", cor: T.inkDim };
  };
  if (!dv2.despertar) {
    return /* @__PURE__ */ import_react2.default.createElement("div", { className: "p-4 md:p-6 space-y-4" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-2xl p-4", style: { background: T.panel, border: `1px solid ${T.amber}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-1", style: { color: T.amberSoft } }, "Ascens\xE3o"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-display text-2xl", style: { color: T.ink } }, "O cosmos ainda n\xE3o te enxerga"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-sm mt-2 leading-relaxed", style: { color: T.inkDim } }, "A trilha dos tr\xEAs est\xE1gios (Servo Escolhido \u2192 Semideus \u2192 Nova Divindade) abre no n\xEDvel ", NIVEL_DESPERTAR, ".", podeDespertar ? " Voc\xEA j\xE1 cruzou esse marco \u2014 o despertar acontece sozinho no pr\xF3ximo turno, ou agora, pelo bot\xE3o abaixo." : " Siga lend\xE1rio at\xE9 l\xE1."), podeDespertar && /* @__PURE__ */ import_react2.default.createElement("div", { className: "mt-3 space-y-2" }, /* @__PURE__ */ import_react2.default.createElement("button", { onClick: onDespertar, className: "tv-btn w-full rounded-xl py-2.5 tv-mono text-xs uppercase tracking-widest", style: { background: T.amber, color: "#1A1206" } }, "\u{1F31F} Despertar agora"), /* @__PURE__ */ import_react2.default.createElement("button", { onClick: onRecalibrar, disabled: recalibrando, className: "tv-btn w-full rounded-xl py-2.5 tv-mono text-xs uppercase tracking-widest", style: { background: T.panelSoft, border: `1px solid ${T.violetSoft}`, color: T.violetSoft } }, recalibrando ? "\u2696 Lendo sua lenda\u2026" : "\u2696 Recalibrar com a IA"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-[11px]", style: { color: T.inkDim } }, "J\xE1 \xE9 divindade na hist\xF3ria? A recalibra\xE7\xE3o l\xEA o livro da aventura e o c\xE2none e ajusta GD, fi\xE9is, dom\xEDnio e pante\xE3o pelo que de fato aconteceu \u2014 nada \xE9 inventado."))));
  }
  return /* @__PURE__ */ import_react2.default.createElement("div", { className: "p-4 md:p-6 space-y-4" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-2xl p-4", style: { background: T.panel, border: `1px solid ${T.amber}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-1", style: { color: T.amberSoft } }, "Seu lugar no cosmos"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex items-baseline gap-2 flex-wrap" }, /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-display text-3xl", style: { color: T.ink } }, "GD ", gd, " \xB7 ", tituloDe(gd)), /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-mono text-xs", style: { color: T.inkDim } }, "n\xEDvel ", nivel)), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-sm mt-1", style: { color: T.inkDim } }, GRAUS[gd].desc), /* @__PURE__ */ import_react2.default.createElement("div", { className: "grid grid-cols-2 gap-2 mt-3" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-xl p-2.5 text-center", style: { background: T.panelSoft, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-display text-xl", style: { color: T.amber } }, dv2.fieis.toLocaleString("pt-BR")), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[9px] uppercase tracking-widest", style: { color: T.inkDim } }, "fi\xE9is")), /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-xl p-2.5 text-center", style: { background: T.panelSoft, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-display text-xl", style: { color: T.violetSoft } }, dv2.pf, /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-mono text-[10px]", style: { color: T.inkDim } }, "/", pfMaximo(dv2))), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[9px] uppercase tracking-widest", style: { color: T.inkDim } }, "pontos de f\xE9 \xB7 +", pfPorDia(dv2), "/dia"))), /* @__PURE__ */ import_react2.default.createElement("div", { className: "mt-4" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-2", style: { color: T.violetSoft } }, "Milagres \u2014 no que gastar a f\xE9"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "space-y-1.5" }, MILAGRES.map((m) => {
    const liberado = m.gd <= gd;
    const podePagar = (dv2.pf || 0) >= m.pf;
    ...
    return /* @__PURE__ */ import_react2.default.createElement(
      "button",
      {
        key: m.id,
        disabled: !ativo,
        onClick: () => onMilagre && onMilagre(m.id),
      ...
      /* @__PURE__ */ import_react2.default.createElement("span", { style: { fontSize: 15 } }, m.icone),
      /* @__PURE__ */ import_react2.default.createElement("span", { className: "flex-1 min-w-0" }, /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-body text-sm", style: { color: T.ink } }, m.nome), /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-body text-xs block", style: { color: T.inkDim } }, m.desc)),
      /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-mono text-[10px] shrink-0 text-right", style: { color: podePagar && liberado ? T.violetSoft : T.inkDim } }, m.pf, " PF", !liberado && /* @__PURE__ */ import_react2.default.createElement("span", { className: "block", style: { color: T.inkDim } }, "GD ", m.gd))
    );
  }))), prox ? /* @__PURE__ */ import_react2.default.createElement("div", { className: "mt-3" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex justify-between tv-mono text-[10px] mb-1", style: { color: T.inkDim } }, /* @__PURE__ */ import_react2.default.createElement("span", null, "rumo a GD ", prox.gd, " \xB7 ", prox.titulo), /* @__PURE__ */ import_react2.default.createElement("span", null, "faltam ", prox.falta.toLocaleString("pt-BR"), " fi\xE9is")), /* @__PURE__ */ import_react2.default.createElement("div", { className: "h-1.5 rounded-full overflow-hidden", style: { background: T.panelSoft } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "h-full rounded-full", style: { width: `${Math.round(prox.progresso * 100)}%`, background: T.amber } }))) : /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] mt-3 uppercase tracking-widest", style: { color: T.amber } }, "Topo da escala \u2014 n\xE3o h\xE1 mais degraus acima de voc\xEA."), (dv2.dominio || dv2.patrono) && /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-sm mt-3", style: { color: T.ink } }, dv2.dominio && /* @__PURE__ */ import_react2.default.createElement("div", null, "\u{1F30C} Dom\xEDnio: ", /* @__PURE__ */ import_react2.default.createElement("b", null, dv2.dominio)), dv2.patrono && /* @__PURE__ */ import_react2.default.createElement("div", null, "\u{1F64F} Patrono: ", /* @__PURE__ */ import_react2.default.createElement("b", null, dv2.patrono)))), /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-2xl p-4", style: { background: T.panel, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-1", style: { color: T.violetSoft } }, "A Regra do Degrau"), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-xs leading-relaxed", style: { color: T.inkDim } }, "Cada degrau de diferen\xE7a de GD d\xE1 ", /* @__PURE__ */ import_react2.default.createElement("b", { style: { color: T.ink } }, "+2 ao mais forte e \u22122 ao mais fraco"), " em ataques, defesas e resist\xEAncias (o sistema aplica nos dados). Mortais n\xE3o ferem divindades de GD 3+ sem artefato lend\xE1rio ou b\xEAn\xE7\xE3o. F\xE9 se ganha com feitos testemunhados, santu\xE1rios e convers\xF5es \u2014 e se gasta em milagres (pequeno ~5 PF, m\xE9dio ~20, grande ~50).")), (() => {
    const cidades = mapa2 && mapa2.cidades || [];
    if (!cidades.length) return null;
    const dev2 = devocao || { cidades: {} };
    const num = resumoNumerico(mapa2, dev2);
    const ranking = cidades.map((c) => ({ c, fe: feDaCidade(dev2, c.nome), est: estadoFe(c, dev2) })).filter((o) => o.fe >= 5 || o.est.chave === "herege" || o.est.chave === "hostil").sort((a, b) => b.fe - a.fe).slice(0, 8);
    return /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-2xl p-4", style: { background: T.panel, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-2", style: { color: T.violetSoft } }, "A f\xE9 no mapa"), !ranking.length ? /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-sm italic", style: { color: T.inkDim } }, "Seu nome ainda n\xE3o \xE9 rezado em cidade nenhuma", num.andarilhos ? ` \u2014 s\xF3 ${num.andarilhos.toLocaleString("pt-BR")} andarilhos pelas estradas` : "", ". Erga um santu\xE1rio num dom\xEDnio seu (painel Gest\xE3o \u2192 Dom\xEDnios) ou fa\xE7a algo que uma cidade inteira testemunhe.") : /* @__PURE__ */ import_react2.default.createElement(import_react2.default.Fragment, null, /* @__PURE__ */ import_react2.default.createElement("div", { className: "space-y-1.5" }, ranking.map(({ c, fe, est }) => {
      const t = temploDe(temploDaCidade(dev2, c.nome));
      return /* @__PURE__ */ import_react2.default.createElement("div", { key: c.nome, className: "rounded-lg px-3 py-2", style: { background: T.panelSoft, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-body text-sm", style: { color: T.ink } }, t ? `${t.icone} ` : "", c.nome), /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-mono text-[9px] px-1.5 py-0.5 rounded shrink-0", style: { color: est.cor, border: `1px solid ${est.cor}` } }, est.rotulo)), /* @__PURE__ */ import_react2.default.createElement("div", { className: "h-1 rounded-full overflow-hidden mt-1.5", style: { background: T.panel } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "h-full rounded-full", style: { width: `${Math.round(fe)}%`, background: est.cor } })), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[9px] mt-0.5", style: { color: T.inkDim } }, Math.round(fe), "% \xB7 \u2248", fieisDaCidade(c, dev2).toLocaleString("pt-BR"), " fi\xE9is", t ? ` \xB7 ${t.nome} (+${t.pf} PF/dia)` : ""));
    })), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-xs mt-2", style: { color: T.inkDim } }, num.templos, " templo(s) rendem ", num.feDia, " PF por dia. Onde h\xE1 templo a f\xE9 cresce sozinha; onde n\xE3o h\xE1, m\xEDngua. A devo\xE7\xE3o tamb\xE9m viaja pelas estradas: cidade vizinha de cidade devota come\xE7a a ouvir falar.")));
  })(), /* @__PURE__ */ import_react2.default.createElement("div", { className: "rounded-2xl p-4", style: { background: T.panel, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] uppercase tracking-widest mb-2", style: { color: T.amberSoft } }, "Pante\xE3o conhecido"), dv2.panteao.length === 0 ? /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-sm italic", style: { color: T.inkDim } }, "Nenhuma divindade se revelou ainda. Os geradores do mundo apresentar\xE3o deuses conforme sua ascens\xE3o avan\xE7a.") : /* @__PURE__ */ import_react2.default.createElement("div", { className: "space-y-2" }, dv2.panteao.map((d) => {
    const cmp = comparar(d.gd);
    const imune = imunePorEscopo(gd, d.gd);
    return /* @__PURE__ */ import_react2.default.createElement("div", { key: d.id || d.nome, className: "rounded-xl p-3", style: { background: T.panelSoft, border: `1px solid ${T.line}` } }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex items-center justify-between gap-2 flex-wrap" }, /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-display text-lg", style: { color: T.ink } }, d.icone, " ", d.nome, " ", /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-body text-sm", style: { color: T.inkDim } }, d.dominio)), /* @__PURE__ */ import_react2.default.createElement("span", { className: "tv-mono text-[10px] px-1.5 py-0.5 rounded", style: { border: `1px solid ${cmp.cor}`, color: cmp.cor } }, cmp.txt)), /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-mono text-[10px] mt-1", style: { color: T.inkDim } }, "GD ", d.gd, " \xB7 ", tituloDe(d.gd), " \xB7 ", (d.fieis || 0).toLocaleString("pt-BR"), " fi\xE9is \xB7 culto: ", d.culto), d.nota && /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-xs mt-1 italic", style: { color: T.inkDim } }, d.nota), imune && /* @__PURE__ */ import_react2.default.createElement("div", { className: "tv-body text-[11px] mt-1", style: { color: T.danger } }, "\u26A0 Imune ao seu dano comum \u2014 cres\xE7a antes de desafi\xE1-lo (ou enfraque\xE7a a f\xE9 dele)."));
  }))), /* @__PURE__ */ import_react2.default.createElement("button", { onClick: onRecalibrar, disabled: recalibrando, className: "tv-btn w-full rounded-xl py-2.5 tv-mono text-[10px] uppercase tracking-widest", style: { background: T.panelSoft, border: `1px solid ${T.violetSoft}`, color: T.violetSoft } }, recalibrando ? "\u2696 Lendo sua lenda\u2026" : "\u2696 Recalibrar ascens\xE3o com a IA"));
}

// .render-teste.jsx
var geo = gerarGeografia("taverna|Render");
var mapa = { cidades: geo.cidades, faccoes: [{ nome: "Ordem do Corvo", relacao: "aliada", lider: "Sar" }], rotas: geo.rotas, regioes: geo.regioes, continente: geo.continente };
mapa.cidades[0].relacao = "jogador";
mapa.cidades[0].sede = true;
var panteao = gerarPanteaoInicial({ lugar: mapa.cidades[0].nome }, 1);
var dv = garantirDivindade({ despertar: true, fieis: 12e3, pf: 40, panteao, dominio: "da Forja" });
var dev = garantirDevocao(null, mapa, dv);
dev = erguerTemplo(dev, mapa.cidades[0].nome, 1).devocao;
dev = depositarFieis(dev, mapa, mapa.cidades[1].nome, 3e3, 2).devocao;
dv = { ...dv, fieis: fieisTotais(mapa, dev) };
var casos = [
  ["PainelMapa (pol\xEDtica)", /* @__PURE__ */ import_react3.default.createElement(PainelMapa, { mapa, faccaoJogador: "Ordem", cidadeAtual: mapa.cidades[0].nome, devocao: dev, divindade: dv })],
  ["PainelMapa (sem despertar)", /* @__PURE__ */ import_react3.default.createElement(PainelMapa, { mapa, faccaoJogador: "Ordem", cidadeAtual: "", devocao: void 0, divindade: garantirDivindade(null) })],
  ["PainelMapa (mapa vazio)", /* @__PURE__ */ import_react3.default.createElement(PainelMapa, { mapa: { cidades: [] }, faccaoJogador: "", cidadeAtual: "", devocao: dev, divindade: dv })],
  ["PainelAscensao (desperto)", /* @__PURE__ */ import_react3.default.createElement(PainelAscensao, { divindade: dv, nivel: 18, mapa, devocao: dev, onMilagre: () => {
  } })],
  ["PainelAscensao (mortal)", /* @__PURE__ */ import_react3.default.createElement(PainelAscensao, { divindade: garantirDivindade(null), nivel: 9, mapa, devocao: dev })],
  ["PainelAscensao (sem devo\xE7\xE3o)", /* @__PURE__ */ import_react3.default.createElement(PainelAscensao, { divindade: dv, nivel: 20, mapa, devocao: void 0 })],
  ["PainelAscensao (sem mapa)", /* @__PURE__ */ import_react3.default.createElement(PainelAscensao, { divindade: dv, nivel: 20, mapa: null, devocao: dev })]
];
var falhas = 0;
for (const [nome, el] of casos) {
  try {
    const html = (0, import_server.renderToString)(el);
    console.log(`OK   ${nome.padEnd(30)} ${html.length} bytes`);
  } catch (e) {
    falhas++;
...
console.log(falhas ? `
${falhas} painel(\xE9is) quebrado(s)` : "\nTodos os pain\xE9is renderizaram");
